...
PREMIUM_COST_GEMS = 100      # больше не показывается как способ ПОКУПКИ (Premium
                             # продаётся только за Stars) — используется только
                             # для старого автопродления за Gems у тех, кто его
                             # включил ещё до перехода на Stars (scheduler.py)
PREMIUM_DAYS = 30
...
# --- ОДНОРАЗОВЫЙ БОНУС ПРИ ПОКУПКЕ PREMIUM ---
# Выдаётся один раз навсегда на аккаунт при первой покупке Premium — любым
# способом (за Gems или за Telegram Stars), повторные покупки/продления его
# не выдают повторно (см. users.premium_bonus_claimed)
PREMIUM_BONUS_PACKS = {"standard": 5, "rare": 4, "epic": 3, "legendary": 3, "mythic": 1}

# --- ОПЛАТА TELEGRAM STARS (валюта XTR, без provider_token) ---
# (алмазы, ⭐)
STARS_GEMS_PACKAGES = [
    (50, 25), (120, 50), (300, 100), (700, 200),
    (2000, 500), (4500, 1000), (10000, 2000),
]
# (монеты, ⭐)
STARS_COINS_PACKAGES = [
    (10000, 250), (25000, 500), (60000, 1000), (150000, 2000),
    (400000, 3000), (1000000, 5000),
]
# редкость -> ⭐ за случайную карту этой редкости (как обычный дроп/пак).
# Limited не входит — у лимитных карт цена в ⭐ задаётся вручную по каждой
# карте отдельно (см. cards.stars_price), их продают как конкретный тайтл,
# а не случайно
STARS_CARD_PRICES = {"Common": 10, "Rare": 150, "Epic": 100, "Legendary": 500, "Mythic": 700}
# (дни Premium, ⭐) — в игре только один уровень подписки (VIP/VIP+ — то же
# самое, что Premium), поэтому просто два варианта длительности
STARS_VIP_PACKAGES = [(7, 100), (30, 300)]
